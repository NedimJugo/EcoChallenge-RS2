class DonationResponse {
  ...
  factory DonationResponse.fromJson(Map<String, dynamic> json) {
    return DonationResponse(
      ...
      stripePaymentStatus: json['stripePaymentStatus'],
  ...
  // Helper methods for Stripe status
  bool get isStripePayment => stripePaymentIntentId != null;
  bool get isPaymentSucceeded => stripePaymentStatus == 'succeeded';
  bool get isPaymentPending => stripePaymentStatus == 'requires_payment_method' ||
                               stripePaymentStatus == 'requires_confirmation' ||
                               stripePaymentStatus == 'processing';
  bool get isPaymentFailed => stripePaymentStatus == 'payment_failed' ||
                              stripePaymentStatus == 'canceled';
  
  // Helper method to get payment status display text
  String get paymentStatusDisplay {
    if (!isStripePayment) return 'Non-Stripe Payment';
...
// Donation Insert Request Model - Updated with Stripe fields
class DonationInsertRequest {
  ...
  final String? paymentMethod;
  final String? paymentReference;
  final String? stripePaymentIntentId;  // Added
  final String? stripePaymentStatus;    // Added
  final String? donationMessage;
  final bool isAnonymous;
  final int statusId;
  final int pointsEarned;
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'paymentMethod': paymentMethod,
      'paymentReference': paymentReference,
      'stripePaymentIntentId': stripePaymentIntentId,  // Added
      'stripePaymentStatus': stripePaymentStatus,      // Added
      'donationMessage': donationMessage,
      'isAnonymous': isAnonymous,
      'statusId': statusId,
      'pointsEarned': pointsEarned,
...
// Donation Update Request Model - Updated with Stripe fields
class DonationUpdateRequest {
  ...
  Map<String, dynamic> toJson() {
    ...
    if (stripePaymentIntentId != null) data['stripePaymentIntentId'] = stripePaymentIntentId;  
    if (stripePaymentStatus != null) data['stripePaymentStatus'] = stripePaymentStatus;        
...
class DonationSearchObject {
  ...
  // Optional: Add Stripe-specific filters
  ...
  Map<String, dynamic> toJson() {
    ...
    if (stripePaymentStatus != null) data['stripePaymentStatus'] = stripePaymentStatus;
    if (isStripePayment != null) data['isStripePayment'] = isStripePayment;
